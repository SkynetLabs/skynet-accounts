...
func (db *DB) RegistryReadCreate(ctx context.Context, user User) (*RegistryRead, error) {
	if user.ID.IsZero() {
...
func (db *DB) RegistryWriteCreate(ctx context.Context, user User) (*RegistryWrite, error) {
	if user.ID.IsZero() {
		return nil, errors.New("invalid user")
	}
	rw := RegistryWrite{
