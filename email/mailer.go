...
but queues it up in the database for future processing. A background thread running
`Sender` is looping over the DB on a timer and taking care to send the
...
succeed, the `SentOn` timestamp is set on each successfully sent message, thus marking it as processed.
Should it fail to send the message, `Sender` will unlock it, so it can be
retried later. If a message fails to get sent more than `maxAttemptsToSend`
times it is marked as `Failed`.
*/
