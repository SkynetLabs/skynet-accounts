...
test-int: test-long start-mongo
	...
	# Run the email_sender tests separately, without parallelism (`-p 1`).
	CONTENDING_SENDERS='run' GORACE='$(racevars)' go test -p 1 -race -v -tags='testing debug netgo' -timeout=300s ./test/email -run=TestContendingSenders -count=$(count)
	-make stop-mongo
